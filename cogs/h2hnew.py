...
def get_data(year, session_type, ignore_dnfs):
    ...
    for c in range(min(schedule.index), max_index + 1):
        ...
        session = fastf1.get_session(
            year, schedule.loc[c, "RoundNumber"], session_type.value
        )
            ...
            if ignore_dnfs == "True":
                if session_type.value == "r":
                    ...
                    for driver in team_results.index:
                        if (
                            (team_results.loc[driver, "ClassifiedPosition"]) in dnf
                        ) or (
                            not (
                                (team_results.loc[driver, "Status"] == "Finished")
                                or ("+" in team_results.loc[driver, "Status"])
                            )
                        ):
                            # for testing
...
def make_plot(data, colors, year, session_type, team_names, filepath, ignore_dnfs):
    plt.clf()
    f1plt.setup_mpl()
    ...
    fig.suptitle(
        f"{year} {session_type.name} Head to Head (Retirements {'Excluded' if ignore_dnfs == 'True' else 'Included'})",
    ...
    for team in data.keys():
        for pairing in data.get(team).keys():
            ...
            sorted_indices = sorted(
                range(len(driver_wins)), key=lambda i: abs(driver_wins[i]), reverse=True
            )
            ...
            color = ""
            if not (
                (colors.get(team).lower() == "nan") or (colors.get(team).lower() == "")
            ):
    ...
    try:
        ...
        if not (os.path.exists(file_path)):
            ...
            try:
                ...
                make_plot(
                    data, colors, year, session_type, names, file_path, ignore_dnfs
                )
            except:
                return (
                    em.ErrorEmbed(
                        "Error while drawing", error_message=traceback.format_exc()
                    ),
                    None,
                )
        ...
        if ignore_dnfs == "True":
            return (
                em.Embed(
                    ...
                    footer="All retirements excluded",
                ),
                file,
            )
        return (
...
class h2hnew(commands.Cog):
    ...
    @app_commands.command(
        name="h2h",
        description="See head to head stats of specific drivers or teammate pairings.",
    )
    ...
    async def h2hnew(
        ...
        dc_embed, file = await loop.run_in_executor(
            None, get_embed, self, year, session_type, ignore_dnfs.value
        )
