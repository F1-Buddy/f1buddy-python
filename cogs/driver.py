    ...
    try:
        ...
        if "anurag" in driver:
            message_embed.set_image(
                url="https://avatars.githubusercontent.com/u/100985214?v=4"
            )
            message_embed.title = "friend anurag"
            return message_embed
        if driver == "saul":
            message_embed.set_image(
                url="https://i.kym-cdn.com/entries/icons/original/000/040/009/3dsaulcover.jpg"
            )
        ...
        if index == -1:
            ...
            message_embed.url = wikipedia.WikipediaPage(
                title=wikipedia.search(driver, results=1)[0]
            ).url
            # message_embed.add_field(name = "Nationality", inline = True, image=f"https:{driver_data[index]['nationality']}")
            message_embed.description += (
                "**:calendar_spiral: Seasons Completed:** "
                + str(driver_data[index]["seasons_completed"])
            )
            message_embed.description += "\n**:trophy: Championships:** " + str(
                driver_data[index]["championships"]
            )
            message_embed.description += "\n**:checkered_flag: Entries:** " + str(
                driver_data[index]["entries"]
            )
            message_embed.description += "\n**:race_car: Starts:** " + str(
                driver_data[index]["starts"]
            )
            message_embed.description += "\n**:stopwatch: Poles:** " + str(
                driver_data[index]["poles"]
            )
            message_embed.description += "\n**:first_place: Wins:** " + str(
                driver_data[index]["wins"]
            )
            message_embed.description += "\n**:medal: Podiums:** " + str(
                driver_data[index]["podiums"]
            )
            message_embed.description += "\n**:rocket: Fastest Laps:** " + str(
                driver_data[index]["fastest_laps"]
            )
            message_embed.description += (
                "\n**:chart_with_upwards_trend: Points:** "
                + str(driver_data[index]["points"])
            )
            message_embed.set_thumbnail(
