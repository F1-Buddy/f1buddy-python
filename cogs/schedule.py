        ...
        if off_season:
            ...
            if schedule.loc[next_event, "EventFormat"] == "conventional":
                converted_session_times = {
                    f":one: {schedule.loc[next_event, 'Session1']}": schedule.loc[
                        next_event, "Session1DateUtc"
                    ],
                    f":two: {schedule.loc[next_event, 'Session2']}": schedule.loc[
                        next_event, "Session2DateUtc"
                    ],
                    f":three: {schedule.loc[next_event, 'Session3']}": schedule.loc[
                        next_event, "Session3DateUtc"
                    ],
                    f":stopwatch: {schedule.loc[next_event, 'Session4']}": schedule.loc[
                        next_event, "Session4DateUtc"
                    ],
                    f":checkered_flag: {schedule.loc[next_event, 'Session5']}": schedule.loc[
                        next_event, "Session5DateUtc"
                    ],
                }
            else:
                converted_session_times = {
                    ...
                    f":stopwatch: {schedule.loc[next_event, 'Session2']}": schedule.loc[
                        next_event, "Session2DateUtc"
                    ],
                    f":stopwatch: {schedule.loc[next_event, 'Session3']}": schedule.loc[
                        next_event, "Session3DateUtc"
                    ],
                    f":race_car: {schedule.loc[next_event, 'Session4']}": schedule.loc[
                        next_event, "Session4DateUtc"
                    ],
                    f":checkered_flag: {schedule.loc[next_event, 'Session5']}": schedule.loc[
                        next_event, "Session5DateUtc"
            ...
            for key in converted_session_times.keys():
                date_object = (
                    converted_session_times.get(key)
                    .tz_localize("UTC")
            ...
            for key in converted_session_times.keys():
                ...
                tdelta = int(
                    (curr_time - pd.Timestamp("1970-01-01").tz_localize("UTC"))
                    / pd.Timedelta("1s")
                )
            ...
            emoji = ""
            alpha2 = (
                coco.convert(names=schedule.loc[next_event, "Country"], to="ISO2")
            ).lower()
            ...
            race_ts = int(
                (
                    converted_session_times.get(f":checkered_flag: Race")
                    - pd.Timestamp("1970-01-01").tz_localize("UTC")
                )
                / pd.Timedelta("1s")
            )
            ...
            title_string = (
                "Race Schedule for " + emoji + "**" + race_name + "**" + emoji
            )
            ...
            if race_within_3days:
                weather_string, precip_string = get_weather_data(
                    f"{schedule.loc[next_event, 'Location']}, {schedule.loc[next_event, 'Country']}"
                )
                days = "Friday\nSaturday\nSunday"
                dc_embed.embed.add_field(
                    name="Weather Forecast:", value="", inline=False
                )
                dc_embed.embed.add_field(name="Day", value=days, inline=True)
                dc_embed.embed.add_field(
                    name="Temperature", value=weather_string, inline=True
                )
                dc_embed.embed.add_field(
                    name="Precipitation", value=precip_string, inline=True
                )
            else:
                dc_embed.embed.set_footer(
                    text="Weather forecast available within 72 hours of race*"
                )
            return dc_embed
