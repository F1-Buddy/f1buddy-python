...
def get_event_data(session_type, year, category, ignore_dnfs):
    ...
    for i in range(first_round, last_round + 1):
        try:
            ...
            for driver in event.drivers:
                ...
                if (
                    isinstance(currDriver_position, str)
                    and currDriver_position.isnumeric()
                ):
                ...
                if ignore_dnfs == "True":
                    if not (
                        "finished" in results.loc[driver, "Status"].lower()
                        or "+" in results.loc[driver, "Status"].lower()
                    ):
                        skip_driver = True
                ...
                if not skip_driver:
                    ...
                    if str(currDriver_position) != "nan":
                        driver_racesParticipated.update(
                            {
                                currDriver_abbreviation: driver_racesParticipated.get(
                        ...
                        driver_positions.update(
                            {
                                currDriver_abbreviation: currDriver_total
                                + currDriver_position
                            }
                        )
    ...
    for key in driver_positions.keys():
        try:
            driver_average.update(
                {key: driver_positions.get(key) / driver_racesParticipated.get(key)}
            )
        except:
            traceback.print_exc()
...
def plot_avg(
    driver_positions, driver_colors, session_type, year, category, filepath, ignore_dnfs
):
    ...
    ax.set_xticklabels(
        range(-1, int(max(driver_positions.values())) + 6),
    ...
    if category.name == "Drivers":
        ...
        fontsize = 10
    ax.set_yticklabels(
        driver_positions.keys(), fontproperties=bold_font, fontsize=fontsize
    )
        ...
        if (curr_color != "nan") and (curr_color != ""):
            ...
            if not annotated:
                plt.figtext(
                    0.91,
                    0.01,
                    "*Some color data is unavailable",
                    ha="center",
                    fontproperties=regular_font,
                )
                annotated = True
    ...
    ab = AnnotationBbox(
        watermark_box, (-0.115, 1.1), xycoords="axes fraction", frameon=False
    )
    ax.add_artist(ab)
    ...
    try:
        ...
        if not (os.path.exists(file_path)):
            ...
            pos, colors = get_event_data(
                ...
                year=year,
                category=category,
                ignore_dnfs=ignore_dnfs,
            )
            # catch any drawing errors
            try:
                plot_avg(
                    pos, colors, session_type, year, category, file_path, ignore_dnfs
                )
        ...
        return (
            em.Embed(
                title=title, description=description, image_url="attachment://image.png"
            ),
            file,
        )
    except Exception as e:
...
class AveragePos(commands.Cog):
    ...
    @app_commands.command(
        name="avgpos",
        description="See average finish position for driver for race/qualifying throughout the year",
    )
    ...
    async def positions(
        ...
        ignore_dnfs: app_commands.Choice[str],
        year: typing.Optional[int],
    ):
        await interaction.response.defer()
        try:
            loop = asyncio.get_running_loop()
            dc_embed, file = await loop.run_in_executor(
                None,
                get_embed_and_image,
                year,
                session_type,
