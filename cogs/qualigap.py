...
message_embed.set_author(
    name="f1buddy",
    icon_url="https://raw.githubusercontent.com/F1-Buddy/f1buddy-python/main/botPics/f1pythonpfp.png",
)
...
def addlabels(x, y, driver_colors, regular_font):
    for i in range(len(x)):
        ...
        if i == 0:
            ...
            string = "No Time"
        plt.text(
            min(max(y), 5) + 0.09, i, string, va="center", fontproperties=regular_font
        )
...
def quali_gap(round, year):
    ...
    try:
        ...
        if (year is None) or ((year > now.year) or (year < 2018)):
            event_year = now.year
            if (cm.currently_offseason()[0]) or (
                cm.latest_quali_completed_index(now.year) == 0
            ):
                event_year -= 1
        else:
            event_year = year
        ...
        if not os.path.exists(
            ...
            driver_names = [
                resultsTable.loc[i, "Abbreviation"] for i in resultsTable.index
            ]
            driver_colors = []
            bar_heights = []
            for i in resultsTable.index:
                # get driver colors
                if year == now.year:
                    driver_color = f1plt.driver_color(
                        resultsTable.loc[i, "Abbreviation"]
                    )
                ...
                if str(resultsTable.loc[i, "Q3"]) != "NaT":
                    bar_heights.append(
                        (resultsTable.loc[i, "Q3"] - fastest_lap).total_seconds()
                    )
                elif str(resultsTable.loc[i, "Q2"]) != "NaT":
                    bar_heights.append(
                        (resultsTable.loc[i, "Q2"] - fastest_lap).total_seconds()
                    )
                elif str(resultsTable.loc[i, "Q1"]) != "NaT":
                    bar_heights.append(
                        (resultsTable.loc[i, "Q1"] - fastest_lap).total_seconds()
                    )
                else:
                    # no time was set
            ...
            plt.barh(driver_names, bar_heights, 0.88, color=driver_colors)
            # label each bar
            ...
            plt.title(
                f"Qualifying Gap for {str(race.date.year)+' '+str(race.event.EventName)}",
...
class QualiGap(commands.Cog):
    ...
    @app_commands.command(
        name="qualigap", description="See driver position changes over the race"
    )
    @app_commands.describe(round="Round name or number (Australia or 3)")
    @app_commands.describe(year="Year")
    async def QualiGap(
        self, interaction: discord.Interaction, round: str, year: typing.Optional[int]
    ):
        ...
        try:
            ...
            message_embed.set_image(
                url="https://media.tenor.com/lxJgp-a8MrgAAAAd/laeppa-vika-half-life-alyx.gif"
            )
