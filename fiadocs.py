...
async def getLatest():
    ...
    fileName = document[
        document.index("/sites/default/files/decision-document/")
        + len("/sites/default/files/decision-document/") :
    ]
    # print(fileName)
    ...
    if not (os.path.exists(f"cogs/fiaDocs/{fileName}")):
        ...
        try:
            while True:
                ...
                images.append(
                    discord.File(
                        f"{filePath}/{page_num}.png", filename=f"{page_num}.png"
                    )
                )
                page_num += 1
        except ValueError:
    ...
    return (
        images,
        fileName[:-4],
        ("https://www.fia.com/" + document.replace(" ", "%20")),
    )
        ...
        try:
            ...
            if images:
                message = await channel.send(
                    embed=em.Embed(
                        title="Latest FIA Doc", description=f"[{docName}]({doc_url})"
                    ).embed,
                    files=images,
                )
